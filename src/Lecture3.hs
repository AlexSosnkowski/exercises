...
next day
  ...
  | otherwise = succ day
--need to solve the challenges here
...
No because it always requires a single element and with this version of appending lists we don't have a 
special placeholder that we can exclude
...
appendDiff3 x y z 
  ...
  | x /= y && (x == z || y == z) = x <> y  
  | otherwise = x 
  -- there has to be a better / more elegant way to do this 
...
-- instance Foldable Weekday where -- no real idea of combining i.e not semigroup 
-- instance Foldable Gold where -- no gold isn't a kind 
-- instance Foldable Reward where -- no not a kind 
...
instance Foldable List1 where
  ...
  -- is this even a monoid???

-- instance Foldable Treasure where -- yes
instance Foldable Treasure where
  foldr :: (a -> b -> b) -> b -> Treasure a -> b 
  foldr _ b_init NoTreasure = b_init 
  foldr f b_init (SomeTreasure t) = f t b_init 
